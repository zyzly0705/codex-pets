...
const sourcePath = path.join(
  repoRoot,
  'assets-src/yoyo/reference/rig/yoyo-standing-clean2d-v3-alpha.png',
);
const outDir = path.join(repoRoot, 'assets-src/yoyo/rig/live2d-yoyo-v5');

const docWidth = 1448;
const docHeight = 1086;
...
const oldCharacterBounds = { left: 419, top: 121, width: 409, height: 893 };
const newCharacterBounds = { left: 517, top: 109, width: 380, height: 844 };
const rigScale = {
  x: newCharacterBounds.width / oldCharacterBounds.width,
  y: newCharacterBounds.height / oldCharacterBounds.height,
};

function transformBox(box) {
  return {
    left: Math.round(newCharacterBounds.left + (box.left - oldCharacterBounds.left) * rigScale.x),
    top: Math.round(newCharacterBounds.top + (box.top - oldCharacterBounds.top) * rigScale.y),
    width: Math.max(1, Math.round(box.width * rigScale.x)),
    height: Math.max(1, Math.round(box.height * rigScale.y)),
  };
}

function transformRigTree(nodes) {
  return nodes.map((node) => {
    if (node.children) return { ...node, children: transformRigTree(node.children) };
    const originalBox = node.box;
    const box = transformBox(originalBox);
    return {
      ...node,
      originalBox,
      box,
      maskScale: {
        x: box.width / originalBox.width,
        y: box.height / originalBox.height,
      },
    };
  });
}

rigTree = transformRigTree(rigTree);
...
function maskBuffer(width, height, svgMarkup, scale = { x: 1, y: 1 }) {
  return Buffer.from(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      <g fill="white" transform="scale(${scale.x} ${scale.y})">${svgMarkup}</g>
    </svg>`,
  );
}
...
async function extractLayer(source, layer) {
  ...
  if (layer.maskSvg) {
    image = image.composite([{ input: maskBuffer(layer.box.width, layer.box.height, layer.maskSvg, layer.maskScale), blend: 'dest-in' }]);
  }
...
async function approvedSourcePreview() {
    ...
    phases: [
      {
        ...
        label: 'Import PSD',
        tasks: [
          'Import yoyo-live2d-rig-v5.psd into Cubism.',
...
async function main() {
  ...
  const previewBuffer = await approvedSourcePreview();
  ...
  fs.writeFileSync(path.join(outDir, 'yoyo-live2d-rig-v5.psd'), writePsdBuffer(psd));
  ...
  const manifest = {
    ...
    notes: [
      'This v5 pack is rebuilt from the user-approved V3 Yoyo source with cute 3-5 year old proportions, large round head, and short childlike limbs.',
      'Layer boxes are transformed from the v4 identity-preserving rig to the approved v3 character bounds before extraction.',
      'V5 keeps the natural face_base from the approved art to avoid visible skin patch seams; expression layers are additive controls for runtime and future hand-cleaned Live2D work.',
