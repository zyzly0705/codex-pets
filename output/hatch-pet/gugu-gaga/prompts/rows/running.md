Create a single horizontal sprite strip for the Codex app digital pet `gugu-gaga` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Original meme-like chibi penguin, round black-and-white body, tiny flippers, small orange beak and feet, dot eyes, goofy and affectionate, not copied from any existing IP..
